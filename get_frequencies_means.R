#GET FREQUENCIES OF SURVEY ANSERS AGGREGATED FOR SETTLEMENT TYPE (IDP/HC) AND AREA LEVEL AS WIDE FORMAT [UNWEIGHTED, ONLY FOR SAME AEREA LEVEL AS STRATA]
library(reshape2)
...
start_time <- Sys.time()                                                                                                #timer
  ...
  for (k in 1:length(u_distr_idp)){                                                                                     #loop over all areas
    ...
    means<-data.frame(sapply(df_num_idp, function(x) mean(x, na.rm=T)))                                                 #compute means
    table_num_idp <- cbind(variables=rownames(means), data.frame(means, row.names=NULL))                                #save variables and means as data frame
...
for (k in 1:length(u_distr_idp)){                                                                                       #loop over all areas
  df_num_non_idp<-df_num[df_num$area_level==u_distr_idp[k] & df_num$settlement_type=="HC",]                             #subset with only survey with area and IDP                          
  means<-data.frame(sapply(df_num_non_idp, function(x) mean(x, na.rm=T)))                                               #compute means
  table_num_non_idp <- cbind(variables=rownames(means), data.frame(means, row.names=NULL))                              #save variables and means as data frame
  table_num_non_idp$settlement_type<-"HC"                                                                               #add column with info about settlement-type
  table_num_non_idp$area<- u_distr_idp[k]                                                                               #add column with area
